        ...
        <div className="mypage-container">
            ...
            {joinedGroups.length > 0 ? (
                joinedGroups.map(group => (
                    <div key={group.id}>
                        <p>{group.name} <button onClick={() => handleWithdrawFromStudy(group.id)}>탈퇴</button></p>
                    </div>
                ))
            ) : (
                <p>스터디에 가입하세요!</p>
            ...
            {joinedGroups.length === 0 ? (
                ...
                recentPosts.map(post => (
                    <div key={post.id}>
                        <p>{post.studyName} - {post.title} <button onClick={() => handleEditPost(post.id)}>수정</button> <button onClick={() => handleDeletePost(post.id)}>삭제</button></p>
                    </div>
                ))
            ) : (
            ...
            {joinedGroups.length === 0 ? (
                ...
                recentAnswers.map(answer => (
                    <div key={answer.id}>
                        <p>{answer.studyName} - {answer.postTitle} <button onClick={() => handleEditPost(answer.postId)}>수정</button> <button onClick={() => handleDeletePost(answer.postId)}>삭제</button></p>
                    </div>
                ))
            ) : (
