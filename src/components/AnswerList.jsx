...
import { createAnswer, deleteAnswer, getAnswers, updateAnswer } from "../services/apiService";
...
const AnswerList = ({ questionId }) => {
    ...
    const loadMoreAnswers = async (id) => {
        try {
            ...
            setAnswers((prevAnswers) => {
                ...
                return Array.from(new Map(allAnswers.map((answer) => [answer.answerId, answer])).values());
            });
        ...
        try {
            ...
            setAnswers((prevAnswers) =>
                prevAnswers.map((answer) =>
                    answer.answerId === answerId ? { ...answer, content: updatedAnswer.content } : answer
                )
            );
    ...
    return (
        <div>
            <p className="answer-count" ref={scrollUpRef}>댓글 {count}</p>
            {answers && answers.map((answer, index) => (
                <div className="answer-list" key={answer.answerId} ref={(el) => (answerRefs.current[index] = el)}>
                    ...
                    scrollToRef={() => answerRefs.current[index]?.scrollIntoView({ behavior: 'smooth', block: 'center' })} />
                </div>
            ))}
            {!isLastPage && (
                <div className="answer-list more-answer" onClick={() => loadMoreAnswers(questionId)}>
                    더보기 ...
                </div>
            )}
            <div className="answer-input-section">
                <textarea placeholder="댓글을 남겨보세요" className="answer-input" value={answer} onClick={isLogin} onChange={(e) => setAnswer(e.target.value)}/>
                {userId !== "" ? (
